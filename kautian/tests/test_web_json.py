...
# Decode one table at a time, each from a document holding only that table, so
# nothing but 詞目 and 義項 is ever retained across steps. This is the shape a
# client uses to keep peak memory near the largest single table.
SPLIT_DRIVER = """
import { readFileSync, writeFileSync } from "node:fs";
...
for (const entry of document.tables) {
  const one = { format: document.format, model: document.model, tables: [entry] };
  context = decodeColumns(one, { only: [entry.name], context });
}
const tables = {};
for (const name of Object.keys(context)) tables[name] = rows(context[name]);
...
class TestWebDocument(unittest.TestCase):
    ...
    def test_a_synthetic_database_decodes_to_the_same_rows(self):
        expected, decoded = self.roundtrip(self.ROWS)
        self.assertEqual(decoded, expected)

    def test_decoding_one_table_at_a_time_gives_the_same_rows(self):
        """A client that never holds the whole document must get the same data."""
        workspace = Path(tempfile.mkdtemp())
...
class TestWebDocumentAgainstTheRealDatabase(unittest.TestCase):
    ...
    def test_decoding_one_table_at_a_time_gives_the_same_rows(self):
        ...
        document = webjson.to_document(schema, data, header)
        with tempfile.TemporaryDirectory() as workspace:
            decoded = decode_with_node(document, Path(workspace), SPLIT_DRIVER)
        self.assertEqual(decoded, expected_rows(schema, data))
